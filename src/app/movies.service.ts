...
export class MoviesService {
  ...
  //sends back all movies available for purchase
  getAllMovies(){
    return ALLMOVIES;
  }
  //sends back the movies in the user's personal collection
  getMyMovies(){
    return MYMOVIES;
  }

//The ability to add  movies from the user's collection
  addToMyMovies(movie:Movie){
    MYMOVIES.push(movie);
    for (let i =0 ; i < ALLMOVIES.length ; i++){
      if (movie.id==ALLMOVIES[i].id)
...
//The ability toremove movies from the user's collection
